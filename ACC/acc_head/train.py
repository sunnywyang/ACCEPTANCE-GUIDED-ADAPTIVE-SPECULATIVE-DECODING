...
class PrintLossCallback(TrainerCallback):
    def on_log(self, args, state, control, logs=None, **kwargs):
        if state.is_local_process_zero:  # 只在主进程打印
            if "loss" in logs:
                print(f"Step {state.global_step} - Loss: {logs['loss']:.4f}")
            if "KL" in logs:
                print(f"Step {state.global_step} - KL: {logs['KL']:.4f}")
...
if __name__ == "__main__":
    ...
    base_model_path="/root/autodl-tmp/Llama27bchathf/"
    ...
    trainer = MyTrainer(
        ...
        callbacks=[PrintLossCallback()]  # 添加回调
    )
